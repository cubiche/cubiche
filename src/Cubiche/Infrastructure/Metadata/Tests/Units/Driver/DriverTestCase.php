...
namespace Cubiche\Infrastructure\Metadata\Tests\Units\Driver;
...
use Cubiche\Infrastructure\Metadata\Tests\Fixtures\Address;
use Cubiche\Infrastructure\Metadata\Tests\Fixtures\User;
        ...
        $this
            ...
            ->then()
                ->exception(function () use ($driver) {
                    $driver->loadMetadataForClass('Cubiche\Infrastructure\Metadata\Tests\Fixtures\Post');
                })->isInstanceOf(MappingException::class)
                ->exception(function () use ($driver) {
                    $driver->loadMetadataForClass('Cubiche\Infrastructure\Metadata\Tests\Fixtures\Blog');
                })->isInstanceOf(MappingException::class)
        ;
        ...
        $this
            ...
            ->then()
                ...
                ->string($classMetadata->propertyMetadata('addresses')->getMetadata('type'))
                    ->isEqualTo('ArrayList[Cubiche\Infrastructure\Metadata\Tests\Fixtures\Address]')
                ->string($classMetadata->propertyMetadata('friends')->getMetadata('type'))
                    ->isEqualTo('ArraySet[Cubiche\Infrastructure\Metadata\Tests\Fixtures\User]')
            ->and()
            ->when($classMetadata = $driver->loadMetadataForClass(User::class))
            ->then()
                ->object($classMetadata)
